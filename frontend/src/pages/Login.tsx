...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (errorParam) {
      const errorMessages: Record<string, string> = {
        'auth_failed': 'Logowanie nie powiodĹ‚o siÄ™. SprĂłbuj ponownie.',
        'no_code': 'Brak autoryzacyjnego kodu. SprĂłbuj ponownie.',
      }
      setError(errorMessages[errorParam] || `BĹ‚Ä…d logowania: ${errorParam}`)
    }
  }, [])
  ...
  const handleLogin = async () => {
    try {
      ...
      if (data.demoMode) {
        alert('âš ď¸Ź Spotify API nie jest skonfigurowany.\n\nUĹĽyj przycisku "Tryb Demo" aby przetestowaÄ‡ aplikacjÄ™.')
        return
      }
      
      if (!data.url) {
        alert('Nie moĹĽna zalogowaÄ‡ siÄ™ przez Spotify.')
        return
      }
      ...
      alert('BĹ‚Ä…d: nie moĹĽna zalogowaÄ‡ siÄ™ przez Spotify')
    }
  }
  ...
  return (
    <div className="flex items-center justify-center min-h-[80vh]">
      <div className="bg-gray-900 rounded-2xl p-10 w-full max-w-md text-center">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">Zaloguj siÄ™ do waveeProjectBW</h1>
        <p className="text-gray-400 mb-8">
          PoĹ‚Ä…cz swoje konto Spotify lub wyprĂłbuj wersjÄ™ demo
        </p>
        ...
        <button
          ...
          title="Zaloguj siÄ™ kontem Spotify (wymaga konfiguracji API)"
        >
        ...
        <p className="text-gray-500 text-sm mt-6">
          Tryb demo uĹĽywa przykĹ‚adowych danych. Nie wymaga konta Spotify.
        </p>
      </div>
    </div>
  )
